...
- Every model file also gets a **DOCUMENTATION INDEX** element (`xml_builder.build_docindex_element`, id `docindex`) - a Grouping listing every generated `.docx` deliverable relevant to that file (phase-scoped files list just their own phase; the master lists every phase, mirroring the `docindex_master` element already present by hand in the SAP Enterprise Repository). `generate_project.py` builds the checkmarked text from the filenames it actually wrote (`documentation_index_text`, tracked per-phase in `docs_by_phase`), not from `phase_map.py` alone, so it can never drift out of sync with what's really on disk. Declared once per file, referenced twice - a prominent placement on the Template view alongside the legend, and again wherever it naturally falls in the Complete Cross-Reference view.
